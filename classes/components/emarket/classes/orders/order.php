<?php
	...
	use UmiCms\Classes\Components\Emarket\Order\iValidator;
	...
	class order extends umiObjectProxy {
		...
		/** @var Settings экземпляр класса управления настройками */
		protected $settings;
		...
		public static function create($useDummyOrder = false, $domainId = false) {
			...
			if(Service::CookieJar()->get('locate')=='msk')
			{
			  $managerId = 893;
			}
		 ...
		 * @throws selectorException
		 */
		public static function getStatusList(string $statusClass = 'order_status') : array {
			$query = Service::SelectorFactory()
				->createObjectTypeName('emarket', $statusClass);
		...
		public function getComment() {
			return $this->getValue('order_comment');
		}

		/**
		 * Изменяет комментарий к заказу
		 * @param string $comment
		 * @return string
		 */
		public function setComment($comment) {
			$this->setValue('order_comment', $comment);
			return $this;
		}
		...
		/**
		 * Меняет десятичный разделитель с запятой на точку.
		 * @param string $value десятичная дробь в виде строки
		 * @return float
		 */
		private function formatTotalProperty($value) {
		...
		public function getTotalWeight() {
			...
			$totalWeight = $this->formatTotalProperty($totalWeight);

			if (!is_numeric($totalWeight) || $totalWeight == 0) {
				$totalWeight = $this->getSettings()
					->get(Settings::ORDER_SECTION, 'defaultWeight');
			}

			return $this->formatTotalProperty($totalWeight);
		}
		...
		public function getTotalWidth() {
			...
			$totalWidth = $this->formatTotalProperty($totalWidth);

			if (!is_numeric($totalWidth) || $totalWidth == 0) {
				$totalWidth = $this->getSettings()
					->get(Settings::ORDER_SECTION, 'defaultWidth');
			}

			return $this->formatTotalProperty($totalWidth);
		}
		...
		public function getTotalHeight() {
			...
			$totalHeight = $this->formatTotalProperty($totalHeight);

			if (!is_numeric($totalHeight) || $totalHeight == 0) {
				$totalHeight = $this->getSettings()
					->get(Settings::ORDER_SECTION, 'defaultHeight');
			}

			return $this->formatTotalProperty($totalHeight);
		}
		...
		public function getTotalLength() {
			...
			$totalLength = $this->formatTotalProperty($totalLength);

			if (!is_numeric($totalLength) || $totalLength == 0) {
				$totalLength = $this->getSettings()
					->get(Settings::ORDER_SECTION, 'defaultLength');
			}

			return $this->formatTotalProperty($totalLength);
		}
		...
		public function isOrderBasket() {
			...
			$orderStatusNotDefined = !$orderStatus;
			$orderNumberNotDefined = !$this->getNumber();

			if ($orderStatusNotDefined && $orderNumberNotDefined) {
				return true;
			}

			if ($this->isPurchasingNotCompleted()) {
				return true;
			}
		...
		public function setBonusDiscount($bonus) {
			...
			$previousBonus = $this->getBonusDiscount();

			if ($previousBonus > 0) {
				try {
					$customer->returnBonuses($previousBonus);
				} catch (privateException $exception) {
					umiExceptionHandler::report($exception);
					return;
				}

				$this->object->setValue('bonus', 0);
				$this->refresh();
			}
		...
		public function order() {
			$orderStatus = $this->getOrderStatus();
			$paymentInProcess = $this->isPaymentInProcess();
			$purchasingNotCompleted = $this->isPurchasingNotCompleted();

			$isEligible = $orderStatus === null || ($purchasingNotCompleted && !$paymentInProcess);
			...
			$validator = Service::get('OrderValidator');
			$validator->validateForPurchasing($this);
			...
			$this->object->commit();
			customer::get()->freeze();
			Service::CookieJar()->remove('PromoCode');
		...
		/**
		 * Определяет, находится ли заказ на стадии оплаты
		 * @return bool
		 * @throws coreException
		 */
		private function isPaymentInProcess() : bool {
			$paymentId = $this->getPaymentId();
			$payment = payment::get($paymentId);

			switch (true) {
				case $payment instanceof courierPayment:
				case $payment instanceof invoicePayment:
				case $payment instanceof receiptPayment:
					return false;
			}

			$paymentStatus = $this->getPaymentStatus();
			$paymentStatusCode = self::getCodeByStatus($paymentStatus);

			return in_array($paymentStatusCode, ['initialized', 'declined']);
		}

		/**
		 * Определяет, что оформление заказа не завершено
		 * @return bool
		 */
		private function isPurchasingNotCompleted() : bool {
			$orderStatus = $this->getOrderStatus();
			$orderStatusCode = self::getCodeByStatus($orderStatus);

			return in_array($orderStatusCode, ['payment', 'editing']);
		}
