	...
	class EmarketNotification {
		...
		public function getOrderItems(order $order, $suffix) {
			...
			foreach ($order->getItems() as $item) {
				$productPage = $item->getRelatedProduct();
				$link = '';
				...
				$currency = Service::CurrencyFacade()->getByCode('RUR');
				$price = Service::CurrencyFacade()->calculate($item->getTotalActualPrice(),Service::CurrencyFacade()->getDefault(),$currency);
			...
			switch ($changedStatus) {
				case 'status_id': {
					...
					if ($codeName == 'waiting') {
						...
						$status = isset($labels[$key]) ? $labels[$key] : '';
					}
		...
		/*	if($order->manager_id==893){
				$emails = 'msk@tvoybasseyn.ru';
			} 
			*/
			...
			$currency = Service::CurrencyFacade()->getByCode('RUR');
			$price = Service::CurrencyFacade()->calculate($order->getActualPrice(),Service::CurrencyFacade()->getDefault(),$currency);
			...
			$labels = $cmsController
				->getLangConstantList();
			$header = $labels['notification-neworder-subject'] ." (СПб) ". " (#{$order->number})";
			
			if($order->manager_id==893){
				$header = $labels['notification-neworder-subject'] ." (Москва) ". " (#{$order->number})";
			} 
