<?php
	...
	class TemplatePhpExtension extends ViewPhpExtension {
		...
		private $noPhotoPath = '/templates/vsk_shop/img/no_photo.jpg';
		...
		public function getRelation($property) {
			$umiObjects = umiObjectsCollection::getInstance();
			$relation = $umiObjects->getObject($property);
			return $relation;
		}
		...
		public function getFeedback($product = null){ 
			$settings = $this->getSettingsContainer();
			$sel = new selector('objects');
			$sel->types('object-type')->id($settings->getValue('otzyvy'));
			if($product)
				$sel->where('tovar')->equals($product);
			$sel->order('ord')->desc();
			$sel->option('load-all-props')->value(true);
