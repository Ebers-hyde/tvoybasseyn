...
site.filters = (function ($) {
  ...
  $(function () {
    document.querySelectorAll(".filter_param").forEach((param) => {
      titles[param.parentElement.classList[2]] =
        param.firstElementChild.textContent;
    });
  ...
  function init() {
    ...
    function initFilterData() {
      var formSelector = canShowMobileFilters()
        ? ".mobile_filters"
        : ".desktop_filters";
      _$form = $(formSelector);
  ...
  function getFilters() {
    var url =
      _baseUrl + _categoryId + ".json" + "?lang_id=" + window.pageData.lang_id;
   ...
   */
  function getFilterAttribute(name){
	let left = name.split('[');
  ...
  function showResult(data) {
    ...
    document.querySelectorAll(".check_container").forEach((check) => {
      ...
      check.querySelectorAll(".filter_checkbox").forEach((val) => {
        if (i >= 1) {
          document
            .querySelector(`.${getFilterAttribute(val.getAttribute("name"))}`)
            .firstElementChild.classList.add("filter_param--active");
          if (i == 1) {
            ...
            document.querySelector(
              ...
              titles[getFilterAttribute(val.getAttribute("name"))] + ": " + i;
          }
        } else if (i == 0) {
          document
            .querySelector(`.${getFilterAttribute(val.getAttribute("name"))}`)
            .firstElementChild.classList.remove("filter_param--active");
          document.querySelector(
            `.${getFilterAttribute(val.getAttribute("name"))}`
          ).firstElementChild.textContent =
            titles[getFilterAttribute(val.getAttribute("name"))];
        }
      });
      i = 0;
    });
  ...
  function onChange(event) {
    ...
    function getRangeParams() {
      ...
      $fieldList.each(function (i, field) {
        ...
        if (boundary === "from" || boundary === "to") {
          if (boundary === "from") {
            var value =
              $("[data-name='fake_filter[price][from]']").val() / rate;

            $field.removeAttr("disabled");
            $field.val(value);
            params["filter[price][from]"] = value;
          }
          if (boundary === "to") {
            var value = $("[data-name='fake_filter[price][to]']").val() / rate;

            $field.removeAttr("disabled");
            $field.val(value);
            params["filter[price][to]"] = value;
          }
        }
      });
    ...
    function showFilterResultPopUp() {
      ...
      var $container = $field
        .parents(".product_form:eq(0)")
        .siblings(".arrow_product:eq(0)");

      $.get(
        "/templates/demomarket/js/filters/filterPopUp.html",
        function (data) {
  ...
  function bindValueChangeHandler() {
    ...
    switch (true) {
      case fieldType.tag === "input" &&
        (fieldType.type === "radio" || fieldType.type === "checkbox"): {
